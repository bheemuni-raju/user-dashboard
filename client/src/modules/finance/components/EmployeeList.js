...
class EmployeeList extends Component {
    ...
    buildToolbarItems = () => {
        ...
        let toPath = isEmpty(subDepartment) ? `create/finance` : `create/finance/${get(subDepartment, "formattedName")}`;
        const createFlag = validatePermission(user, finance.createFinanceEmployees);

        return (
            <>
                <Link className="btn btn-success btn-sm" to={toPath} hidden={!createFlag}>
    ...
    getList = () => {
        ...
        let { user = {} } = this.props;
        const editFlag = validatePermission(user, finance.editFinanceEmployees);
        ...
        const formatters = () => ({
            nameFormatter: (cell, row) => {
                ...
                if (editFlag) {
                    return <Link
                        to={{
                            pathname: `${row.email}/${subDepartment}/edit`
                        }}
                    >{startCase(cell)}</Link>
                }
                else {
        ...
        return (
            < ByjusGrid
                ...
                modelName="FinanceEmployee"
                pillOptions={{
                    pills,
                    defaultPill
                }}
                gridTitle="Finance Employees"
                gridId="ums_finance_grid"
