...
import { get, capitalize, find, map, sortBy, flattenDeep, isEmpty, remove, camelCase, startCase, isArray } from 'lodash'
...
import { fetchReportingToRoles, getFormattedDateFields, validateDateRange, validateEmail } from 'modules/user/utils/userUtil';
...
class EmployeeForm extends Component {
    ...
    buildReportingToDetailsForm = (userData) => {
        ...
        let { reportingToRoles, initialValues } = fetchReportingToRoles(reportingTo, teamRoles, userRole, status, activationDate, "finance");
        ...
        return (
            <>
                <CommonUserForm ref="commonUserForm"
                    ...
                    loadCommentUrl={`/usermanagement/financeemployee/getComments?email=${get(userData, 'email', '')}`}
                    updateCommentUrl={`/usermanagement/financeemployee/updateComments`}
    ...
    onClickSaveUser = async () => {
        const { commonUserForm, reportingToDetailsForm, permissionDetailsForm } = this.refs;
         ...
         */
        if (commonUserFormDetails && reportingToDetails && permissionDetails) {
            let result = await Confirm();
            if (result) {
                this.saveUser(commonUserFormDetails, reportingToDetails, permissionDetails);
            }
        }
    }

    saveUser = async (commonUserFormDetails, reportingToDetails, permissionDetails) => {
        const { user } = this.props;
        const { operationType, userId, teamDetails, userFormValues = {}, userData = {} } = this.state;
        const { reportingTo } = userData;
        const method = "POST"
        const url = operationType == "create" ? "/usermanagement/financeemployee/createData" : `/usermanagement/financeemployee/updateData`
    ...
    getUserData = (email) => {
        this.setState({ loading: true, error: null })
        callApi(`/usermanagement/financeemployee/readData/${email}`, "GET", null, null, null, true)
            .then(response => {
                this.setState({
                    userData: response,
