...
const AppGroupList = (props) => {
    ...
    const deleteRecord = (record) => {
        ...
        setLoading(true);
        setError(null);
        callApi(`/usermanagement/appgroup/${groupFormattedName}`, 'DELETE', null, null, null, true)
            .then(response => {
    ...
    const updateGroupStatus = async (record, status) => {
        ...
        let url = `/usermanagement/appgroup/${record.appGroupName}`;
        ...
        appGroupFormatter: (cell, row) => {
            ...
            if (canEditAppGroup) {
                return (
                    <div>
                        <Link
                            to={{ pathname: `/settings/app-groups/${row.appGroupName}/edit`, state: { groupData: row } }}
                        >{startCase(cell)}</Link>
                        <div style={{ width: "100%" }}>
    ...
    return (
        <Box>
            <BoxBody loading={loading} error={error}>
                <ByjusGrid
                    isKey="_id"
                    gridId="ums_appgroup_grid"
                    ref={byjusGridRef}
                    toolbarItems={buildToolbarItems()}
                    formatters={formatters()}
                    modelName="AppGroup"
                    gridDataUrl="/usermanagement/appgroup/list"
                />
                {showCreateModal &&
                    <AppGroupModal
