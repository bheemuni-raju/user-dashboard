...
import { addOperators } from "../../../../core/components/advancesearch/AdvanceSearchUtil";

import '../../../../core/components/advancesearch/AdvanceSearch.css';

const mapStateToProps = state => ({
    user: state.auth.user
})

class AssignmentRuleFilters extends Component {
    constructor(props) {
        super(props)
        this.state = {
            columnOptions: [],
            ...
            selectedValue: {},
            searchCriterias: [],
            viewAssignmentRuleFilterModal: true,
            anyCriteria: true,
            allCriteria: false,
            columnList: [],
            selectedValueType: '',
    ...
    componentDidMount = async () => {
        ...
        const { columns, searchCriterias = [], anyCriteria } = this.props
        ...
        if (anyCriteria !== this.state.anyCriteria && anyCriteria !== undefined) {
            this.setState({
                anyCriteria,
                allCriteria: !anyCriteria
            })
        }
    }
    ...
    onClickSaveAssignmentRule = () => {
        ...
        this.setState({ ruleCondition, searchCriterias, anyCriteria, allCriteria: !anyCriteria });
        this.props.onSave(ruleCondition, searchCriterias, anyCriteria);
    ...
    hide = () => {
        this.setState({
            viewAssignmentRuleFilterModal: false
        })
    }
    ...
    renderAssignmentRuleFilterView = () => {
        ...
        return (
            <Fragment>
                ...
                <div className="text-right">
                    <Button color="primary" disabled={searchCriterias.length == 0} onClick={this.onClickSaveAssignmentRule}>
                        <i className="fa fa-search-plus"></i> Save Assignment Rule Filter
                    </Button>
                </div>
            </Fragment>
        )
    ...
    renderForm() {
        const tabs = [{
            title: 'Assignment Rule Filter View',
            component: this.renderAssignmentRuleFilterView()
        }]
        return (
        ...
        return (
            <Fragment>
                <Modal
                    ...
                    backdrop={false}
                    keyboard={true}
                    isOpen={this.state.viewAssignmentRuleFilterModal}
                    toggle={this.hide}
                    backdrop={true}
                    className={classes}
                >
...
export default connect(mapStateToProps, null, null, { forwardRef: true })(AssignmentRuleFilters);
