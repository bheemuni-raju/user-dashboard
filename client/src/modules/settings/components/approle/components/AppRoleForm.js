...
const AppRoleForm = (props) => {
    ...
    const { appRoleFormattedName } = props.match.params;

    const saveAppRole = () => {
        const { isEdit, appRoleFormattedName: appRole } = props
        const url = isEdit ? `/usermanagement/approle/${appRole}` : `/usermanagement/approle`
        const method = isEdit ? "PUT" : "POST"

        if (!appRoleName && !appRoleFormattedName) {
            setValidFlag(false);
        }
    ...
    useEffect(() => {
        ...
        async function getApplicationPermissionModules() {
            setLoading(true);
            const modulesResponse = await callApi(`/usermanagement/permission/permissionTemplate/modules/?app=${appName}`, 'GET', null, null, null, true)
    ...
    useEffect(() => {
        async function getPermissionGroups() {
            const { isEdit, appRoleFormattedName } = props
            if (appRoleFormattedName) {
                setLoading(true);
                const url = `/usermanagement/approle/${appRoleFormattedName}`;
                const appRoleResponse = await callApi(url, 'GET', null, null, null, true)

                const { appRoleName,appName, permissions, description } = appRoleResponse
                if (isEdit) {
                    setDescription(description);
                    setAppRoleName(appRoleName);
                }
