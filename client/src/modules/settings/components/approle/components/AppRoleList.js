...
const AppRoleList = (props) => {
    ...
    const onClickToggle = async (record, value) => {
        ...
        setLoading(true);
        let appRoleUsers = appUsers.filter(user => user.appRoleName === record.appRoleFormattedName);
    ...
    const updateRoleStatus = async (record, appRoleUsers, method, onToggle) => {
        ...
        let url = `/usermanagement/approle/${record.appRoleFormattedName}`;
    ...
    const onClickEdit = (row) => {
        history.push(`app-roles/${row.appRoleFormattedName}/edit`);
    }

    const onClickClone = (row) => {
        history.push(`app-roles/${row.appRoleFormattedName}/clone`);
    }
        ...
        <Box>
            <BoxBody loading={loading} error={error}>
                {appUsersLoaded && <ByjusGrid
                    isKey="_id"
                    gridId="ums_approle_grid"
                    ref={byjusGridRef}
                    toolbarItems={buildToolbarItems()}
                    formatters={formatters()}
                    modelName="AppRole"
                    gridDataUrl={`/usermanagement/approle/list`}
                />}
                <ModalWindow
