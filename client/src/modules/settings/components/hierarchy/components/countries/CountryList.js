...
const CountryList = () => {
    ...
    const deleteRecord = (record) => {
        let countryId = record._id;

        setLoading(true);
        callApi(`/usermanagement/country/${countryId}`, 'DELETE', null, null, null, true)
            .then(response => {
                setLoading(false);
                setError(null);
    ...
    return (
        <Box>
            <BoxBody loading={loading} error={error}>
                <ByjusGrid
                    isKey="_id"
                    gridId="ums_country_grid"
                    ref={byjusGridRef}
                    toolbarItems={buildToolbarItems()}
                    formatters={formatters()}
                    modelName="Country"
                    gridDataUrl="/usermanagement/country/list"
                />
                {showCountryModal &&
                    <CountryModal
