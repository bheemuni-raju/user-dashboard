...
const CampaignList = () => {
  ...
  const deleteRecord = (data) => {
    let campaignId = data._id;

    setLoading(true);
    callApi(`/usermanagement/hierarchy/campaign/${campaignId}`, 'DELETE', null, null, null, true)
      .then(response => {
        setLoading(false);
        setError(null);
  ...
  return (
    <Box>
      <BoxBody loading={loading} error={error}>
        <ByjusGrid
          isKey="_id"
          gridId="ums_campaign_grid"
          ref={byjusGridRef}
          toolbarItems={buildToolbarItems()}
          formatters={formatters()}
          modelName="Campaign"
          gridDataUrl="/usermanagement/hierarchy/campaign/list"
        />
        {showCampaignModal &&
          <CampaignModal
