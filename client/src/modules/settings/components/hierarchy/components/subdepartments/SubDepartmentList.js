...
const SubDepartmentList = () => {
    ...
    return (
      <Fragment>
        ...
        {createFlag && !selectedRow ?
          ...
          :
          <>
            <Link className="btn btn-success btn-sm" to={{ pathname: `/settings/setup/subdepartments/${get(selectedRow, 'formattedName')}/role`, state: { subDepartmentData: selectedRow } }}>
              <i className="fa fa-users"></i> {' '}Roles
            </Link> {" "}
            <Link className="btn btn-success btn-sm" to={{ pathname: `/settings/setup/subdepartments/${get(selectedRow, 'formattedName')}/team`, state: { subDepartmentData: selectedRow } }}>
  ...
  const deleteRecord = (data) => {
    ...
    setLoading(true);
    callApi(`/usermanagement/hierarchy/subDepartment/${departmentId}`, 'DELETE', null, null, null, true)
      .then(response => {
        setLoading(false);
        setError(null);
  ...
  return (
    <Box>
      <BoxBody loading={loading} error={error}>
        <ByjusGrid
          isKey="_id"
          gridId="ums_subdepartment_grid"
          ref={byjusGridRef}
          toolbarItems={buildToolbarItems()}
          selectRow={selectRowProp}
          formatters={formatters()}
          modelName="SubDepartment"
          gridDataUrl="/usermanagement/hierarchy/subDepartment/list"
        />
