...
const RoleList = (props) => {
    ...
    const deleteRecord = (roleData) => {
        setLoading(true);
        callApi(`/usermanagement/hierarchy-beta/role/${get(roleData, 'id')}`, 'DELETE', null, null, null, true)
            .then(response => {
                setLoading(false);
                setError(null);
    ...
    return (
        <Box>
            ...
            <BoxBody loading={loading} error={error}>
                <ByjusGrid
                    isKey="_id"
                    gridId="ums_role_beta_grid"
                    ref={byjusGridRef}
                    toolbarItems={buildToolbarItems()}
                    formatters={formatters()}
                    modelName="Role"
                    gridDataUrl={`/usermanagement/hierarchy-beta/role/list`}
                />
                {showRoleModal &&
                    <RoleModal
