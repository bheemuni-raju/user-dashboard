...
const DepartmentList = (props) => {
  ...
  const deleteRecord = (data) => {
    ...
    setLoading(true);
    callApi(`/usermanagement/hierarchy-beta/department/${departmentId}`, 'DELETE', null, null, null, true)
      .then(response => {
        setLoading(false);
        setError(null);
  ...
  return (
    <Box>
      <BoxBody loading={loading} error={error}>
        <ByjusGrid
          isKey="_id"
          gridId="ums_department_beta_grid"
          ref={byjusGridRef}
          toolbarItems={buildToolbarItems()}
          formatters={formatters()}
          modelName="Department"
          gridDataUrl="/usermanagement/hierarchy-beta/department/list"
        />
        {showDepartmentModal &&
          <DepartmentModal
