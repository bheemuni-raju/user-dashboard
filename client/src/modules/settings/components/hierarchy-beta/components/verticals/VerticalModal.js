...
class Vertical extends Component {
    ...
    validateVerticalDetails = (formValues = {}) => {
        let { name = "" } = formValues;
    ...
    onClickSave = () => {
        ...
        const verticalForm = this.refs.verticalForm;
        const formValues = verticalForm ? verticalForm.validateFormAndGetValues() : null;
        if (formValues) {
            this.setState({ loading: true });
            const type = data ? "edit" : "add";
            const url = (type == "add") ? `/usermanagement/hierarchy-beta/vertical` : `/usermanagement/hierarchy-beta/vertical/${get(data, 'id')}`;
    ...
    render() {
        ...
        return (
            <ModalWindow
                ...
                heading={`${data ? "Edit" : "Create"} Vertical`}
            >
                {this.buildForm(data)}
            </ModalWindow>
        )
