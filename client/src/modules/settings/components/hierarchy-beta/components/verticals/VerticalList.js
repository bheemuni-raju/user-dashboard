...
const VerticalList = () => {
  ...
  const deleteRecord = (data) => {
    let verticalId = data.id;

    setLoading(true);
    callApi(`/usermanagement/hierarchy-beta/vertical/${verticalId}`, 'DELETE', null, null, null, true)
      .then(response => {
        setLoading(false);
        setError(null);
  ...
  return (
    <Box>
      <BoxBody loading={loading} error={error}>
        <ByjusGrid
          isKey="_id"
          gridId="ums_vertical_beta_grid"
          ref={byjusGridRef}
          toolbarItems={buildToolbarItems()}
          formatters={formatters()}
          modelName="Vertical"
          gridDataUrl="/usermanagement/hierarchy-beta/vertical/list"
        />
        {showVerticalModal &&
          <VerticalModal
