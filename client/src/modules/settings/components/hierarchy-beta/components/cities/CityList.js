...
const CityList = () => {
    ...
    const deleteRecord = (record) => {
        let cityId = record.id;

        setLoading(true);
        callApi(`/usermanagement/hierarchy-beta/city/${cityId}`, 'DELETE', null, null, null, true)
            .then(response => {
                setLoading(false);
                setError(null);
    ...
    const formatters = () => ({
        countryFormatter:(countryFormattedName)=>{
            return startCase(countryFormattedName);
        },
    ...
    return (
        <Box>
            <BoxBody loading={loading} error={error}>
                <ByjusGrid
                    isKey="_id"
                    gridId="ums_city_beta_grid"
                    ref={byjusGridRef}
                    toolbarItems={buildToolbarItems()}
                    formatters={formatters()}
                    modelName="City"
                    gridDataUrl="/usermanagement/hierarchy-beta/city/list"
                />
                {showCityModal &&
                    <CityModal
