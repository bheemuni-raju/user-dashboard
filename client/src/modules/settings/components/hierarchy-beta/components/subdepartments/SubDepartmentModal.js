...
class SubDepartment extends Component {
        ...
        return (
            <>
                <FormBuilder
                    ...
                    validateValues={this.validateSubDepartmentDetails}
                    cols={1}
                />
    ...
    validateSubDepartmentDetails = (formValues = {}) => {
        let { name = "" } = formValues;
    ...
    onClickSave = () => {
        ...
        const subDepartmentForm = this.refs.subDepartmentForm;
        const formValues = subDepartmentForm ? subDepartmentForm.validateFormAndGetValues() : null;

        if (formValues) {
            this.setState({ loading: true });
            const type = data ? "edit" : "add";
            const url = (type == "add") ? `/usermanagement/hierarchy-beta/subDepartment` : `/usermanagement/hierarchy-beta/subDepartment/${get(data, 'id')}`;
    ...
    render() {
        ...
        return (
            <ModalWindow
                ...
                heading={`${data ? "Edit" : "Create"} SubDepartment`}
            >
                {this.buildForm(data)}
            </ModalWindow>
        )
