...
class ReporterList extends Component {
    ...
    getEmployeeDetails = (userData) => {
        ...
        const initialValues = {
            ...
            role: get(userData, 'role.name'),
            tnlId: capitalize(get(userData, 'tnlId'))
        }

        const reportingToFields = reportingTo.map(rep => {
            initialValues[get(rep, 'level')] = get(rep, 'user.email');
            return {
                type: 'readonlytext',
                name: get(rep, 'level'),
                label: get(find(teamRoles, { _id: get(rep, 'user.role') }), 'name')
            };
        })
        const fields = [{
            type: 'readonlytext',
    ...
    getPills = (userExpDetails, userData) => {
        ...
        let pills = reportersRoles && reportersRoles.map(role => {
            const userFilterQuery = [{
                selectedColumn: "reportingTo.user",
                selectedOperator: "in",
                selectedValue: [get(userData, '_id')]
            }];
