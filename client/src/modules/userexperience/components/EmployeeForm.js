...
import { get, find, map, isEmpty, camelCase, startCase, isArray, remove } from 'lodash'
...
class EmployeeForm extends Component {
    ...
    handleOnChange = (selectedValue, name) => {
        let { teamDetails = {}, additionalRegionList, regionList, userData = {} } = this.state;
        let campaigns = get(teamDetails, 'campaigns', []);
        this.setState({ [name]: selectedValue });
        if (name == "subDepartment") {
            this.setState({ role: null });
            const { commonUserForm } = this.refs;
            commonUserForm.getTeamDetails(selectedValue);
            this.buildReportingToDetailsForm(userData);
        }

        if (name == "department") {
            this.setState({ teamDetails: null });
        }

        if (name == "role") {
            this.buildReportingToDetailsForm(userData, selectedValue);
        }

        if (["campaign"].includes(name)) {
            additionalRegionList = campaigns.filter((campaign) => !selectedValue.includes(campaign.formattedName));
            this.setState({ additionalRegionList });
        }

        if (["additionalRegions"].includes(name)) {
            regionList = campaigns.filter((campaign) => !selectedValue.includes(campaign.formattedName));
            this.setState({ regionList });
        }
    }

    buildReportingToDetailsForm = (userData, onChangeRole = "") => {
        let { teamDetails, role, status, activationDate } = this.state;
        if (!isEmpty(onChangeRole)) {
            role = onChangeRole;
        }
        ...
        let { reportingToRoles, initialValues } = fetchReportingToRoles(reportingTo, teamRoles, userRole, status, activationDate, "user_experience");
        ...
        return (
            <>
                <CommonUserForm ref="commonUserForm"
                    ...
                    loadCommentUrl={`/usermanagement/ueemployee/getComments?email=${get(userData, 'email', '')}`}
                    updateCommentUrl={`/usermanagement/ueemployee/updateComments`}
    ...
    onClickSaveUser = async () => {
        ...
        console.log(commonUserFormDetails);
    ...
    saveUser = async (commonUserFormDetails, reportingToDetails) => {
        ...
        const method = "POST"
        const url = operationType == "create" ? "/usermanagement/ueemployee/createData" : `/usermanagement/ueemployee/updateData`
    ...
    formatAdditionalDetails = (commonUserFormDetails = {}) => {
        let languageArray = isEmpty(get(commonUserFormDetails, 'language')) ? [] : get(commonUserFormDetails, 'language').split(",");
        languageArray = languageArray.map((language) => {
            return camelCase(language);
        });

        let tenure = isEmpty(get(commonUserFormDetails, "tenure")) ? "" : get(commonUserFormDetails, "tenure");
        let additionalRegionArray = isEmpty(get(commonUserFormDetails, "additionalRegions")) ? [] : get(commonUserFormDetails, "additionalRegions");
        let nestingEnabledFlag = get(commonUserFormDetails, "nestingEnabled");
        let lsUserName = get(commonUserFormDetails, "lsUserName");
        let fdUserName = get(commonUserFormDetails, "fdUserName");
        let byjusClasses = get(commonUserFormDetails, "byjusClasses");
        let highPriority = get(commonUserFormDetails, "highPriority");
        let capacity = get(commonUserFormDetails, "capacity");
        let weekoffWkOneDayOne = get(commonUserFormDetails, "weekoffWkOneDayOne");
        let weekoffWkOneDayTwo = get(commonUserFormDetails, "weekoffWkOneDayTwo");
        let weekoffWkTwoDayOne = get(commonUserFormDetails, "weekoffWkTwoDayOne");
        let weekoffWkTwoDayTwo = get(commonUserFormDetails, "weekoffWkTwoDayTwo");
        let webSite = get(commonUserFormDetails, "webSite");
        let customerType = get(commonUserFormDetails, "customerType");
        let formattedAdditionalDetails = {
            "leadCount": 0,
            "languageFormattedName": languageArray,
            "tenure": tenure,
            "additionalRegions": additionalRegionArray,
            "nestingEnabled": nestingEnabledFlag,
            "lsUserName": lsUserName,
            "fdUserName": fdUserName,
            "byjusClasses": byjusClasses,
            "highPriority": highPriority,
            "capacity": capacity,
            "weekoffWkOneDayOne": weekoffWkOneDayOne,
            "weekoffWkOneDayTwo": weekoffWkOneDayTwo,
            "weekoffWkTwoDayOne": weekoffWkTwoDayOne,
            "weekoffWkTwoDayTwo": weekoffWkTwoDayTwo,
            "webSite": webSite,
    ...
    getUserData = (email) => {
        this.setState({ loading: true, error: null })
        callApi(`/usermanagement/ueemployee/readData/${email}`, "GET", null, null, null, true)
            .then(response => {
                this.setState({
                    userData: response,
